...
/// Drive the GUI's headless channels probe (Phase 6a, story 04): connect, refresh the transport
/// adapters + instances and enumerate conversations, printing `DAEMON_APP_CHANNELS adapters=N
/// instances=M`. Asserts (via the proxy) the TransportAdapters + TransportInstances + ConvList frames
/// cross the socket against the real daemon. `DAEMON_BIN` removed so it attaches to the running daemon.
pub fn run_gui_channels(
    gui: &std::path::Path,
    socket: &std::path::Path,
    timeout_ms: u32,
) -> Result<ClientRun> {
    ...
    cmd.env("QT_QPA_PLATFORM", "offscreen")
        .env("DAEMON_APP_WAIT_READY", timeout_ms.to_string())
        .env("DAEMON_APP_CHANNELS_PROBE", "1")
        .env_remove("DAEMON_BIN");
    run_with_timeout(cmd, vec![tmp], home, Duration::from_secs(60))
}
